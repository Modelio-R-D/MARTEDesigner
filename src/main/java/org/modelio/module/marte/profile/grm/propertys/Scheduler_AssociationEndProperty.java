...
@objid ("f5d2d092-038a-4976-b47d-1fafc2273520")
public class Scheduler_AssociationEndProperty implements IPropertyContent {
    @objid ("533aa1ce-32d8-45d5-bac6-2780cb19a756")
    private static List<ModelElement> listComputingResource = null;

    @objid ("f001c43e-f7ca-4600-ae05-814306401d98")
    private static List<ModelElement> listMutualExclusionResource = null;

    @objid ("afe2f6c8-bcbb-4c7a-8d15-4b6302418086")
    private static List<ModelElement> listProcessingResource = null;

    @objid ("603b28c7-47c0-40ac-af57-02e77c9acc30")
    private static List<ModelElement> listSchedulableResource = null;

    @objid ("cf1324f9-3428-41f6-ac76-68147582704f")
    @Override
    public void changeProperty(final ModelElement elt, final int row, final String value) {
        if(row == 1){
            ModelUtils.addStringValue(elt, MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_ISPREEMPTIBLE, value);
        }
        else if(row == 2){
            ModelUtils.addStringValue(elt, MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_SCHEDPOLICY, value);
        }
        else if(row == 3){
            ModelUtils.addStringValue(elt, MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_OTHERSCHEDPOLICY, value);
        }
        else if(row == 4){
            ModelUtils.addStringValue(elt, MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_SCHEDULE, value);
        }
        else if(row == 5){
            ...
            ModelUtils.manageMultipleToOneLink(elt,
                    ...
                    possibleTagTypeName,
                    MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_PROCESSINGUNITS,
                    value
            );
        }
        else if(row == 6){
        ...
        //                    MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_HOST, 
                    value
            );
        }
        else if(row == 7){
            ...
            ModelUtils.manageMultipleToOneLink(elt,
                    ...
                    possibleTagTypeName,
                    MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_PROTECTEDSHAREDRESOURCES,
                    value
            );
        }
        else if(row == 8){
            ...
            ModelUtils.manageMultipleToOneLink(elt,
                    ...
                    possibleTagTypeName,
                    MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_SCHEDULABLERESOURCES,
                    value
            );
        }
    }

    @objid ("452d38d0-1c28-43d8-9a8e-1604c4151f56")
    @Override
    public void update(final ModelElement elt, final IModulePropertyTable table) {
        ...
        table.addProperty(MARTEResourceManager.getPropertyName(MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_ISPREEMPTIBLE),ModelUtils.hasTaggedValue(MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_ISPREEMPTIBLE, elt));
        
        String[] types = MARTEEnumerationUtils.getSchedPolicyKind();
        String value_schedPolicy = ModelUtils.getTaggedValue(MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_SCHEDPOLICY, elt);
        if(!(MARTEEnumerationUtils.isSchedPolicyKind(value_schedPolicy))){
            value_schedPolicy = "FixedPriority";
        }
        table.addProperty(MARTEResourceManager.getPropertyName(MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_SCHEDPOLICY),value_schedPolicy, types);
        
        
        String value_otherSchedPolicy = ModelUtils.getTaggedValue(MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_OTHERSCHEDPOLICY, elt);
        if(value_otherSchedPolicy.equals("")){
            value_otherSchedPolicy = "";
        }
        table.addProperty(MARTEResourceManager.getPropertyName(MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_OTHERSCHEDPOLICY),value_otherSchedPolicy);
        
        
        String value_schedule = ModelUtils.getTaggedValue(MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_SCHEDULE, elt);
        if(value_schedule.equals("")){
            value_schedule = "";
        }
        table.addProperty(MARTEResourceManager.getPropertyName(MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_SCHEDULE),value_schedule);
        ...
        table.addProperty(MARTEResourceManager.getPropertyName(MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_PROCESSINGUNITS), value_processingUnits, tab_processingUnits);
        ...
        table.addProperty(MARTEResourceManager.getPropertyName(MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_HOST), value_host, tab_host);
        ...
        table.addProperty(MARTEResourceManager.getPropertyName(MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_PROTECTEDSHAREDRESOURCES), value_protectedSharedRsources, tab_protectedSharedRsources);
        ...
        table.addProperty(MARTEResourceManager.getPropertyName(MARTEDesignerTagTypes.SCHEDULER_ASSOCIATIONEND_SCHEDULER_ASSOCIATIONEND_SCHEDULABLERESOURCES), value_schedulableResources, tab_schedulableResources);
    }
