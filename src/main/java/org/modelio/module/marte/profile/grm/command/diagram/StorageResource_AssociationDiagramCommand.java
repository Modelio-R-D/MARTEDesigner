...
import org.modelio.module.marte.profile.grm.model.StorageResource_Association;

public class StorageResource_AssociationDiagramCommand extends DefaultLinkTool {
    @Override
    ...
    public void actionPerformed(final IDiagramHandle diagramHandle, IDiagramGraphic originNode, IDiagramGraphic targetNode, LinkRouterKind touterType, ILinkPath path) {
        try(ITransaction tr = MARTEModule.getInstance().getModuleContext().getModelingSession().createTransaction("StorageResource_AssociationCommand")){
        
        
        
        
            StorageResource_Association proxy = new StorageResource_Association();
